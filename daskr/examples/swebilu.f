...
C
C The DAE system is solved by SDASKR with a sparse approximate Jacobian
C matrix as the preconditioner for the preconditioned Krylov option of
C SDASKR.  The Jacobian is formed by using finite-differences of residual
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   SETPAR, CINIT, SDASKR, OUTWEB
C
C***END PROLOGUE  SWEBILU
C
      IMPLICIT REAL (A-H,O-Z)
      EXTERNAL RESWEB, RTWEB, SJACILU, SPSOLILU
...
C
C Set unit number for SDASKR error message output.
      CALL XSETUN(LOUT)
C
C Open output files.
...
C
      WRITE(LOUT,20)NS
 20   FORMAT(' SWEBILU: Example program for SDASKR package'//
...
C enough work array lengths.
 ...
 45      FORMAT(' Error return from SSPSETUP: IERR = ',i5)
      ...
      ENDIF
C
C Set remaining method parameters for SDASKR.
...
C
 ...
 140  FORMAT(//'   t',12X,'Ave.c1  NSTEP  NRE  NNI  NLI  NPE  NQ',4X,
     1       'H',10X,'AVLIN')
C
C Loop over output times and call SDASKR.  At each output time,
...
C
      DO 200 IOUT = 0,NOUT
C
 150    CALL SDASKR (RESWEB, NEQ, T, CC, CCPRIME, TOUT, INFO, RTOL,ATOL,
